...
BIP39 BIP39::generateSecureEntropy() {
    ...
    if (ret) {
#ifdef _WIN32
            ret = NT_SUCCESS(BCryptGenRandom(bcrypt_algo, bytes, (long)size, 0));
#else
...
std::vector<uint8_t> BIP39::seed( Algo algo, const std::string &pass, const std::string &salt, uint32_t iterations, size_t outKeySize) {
