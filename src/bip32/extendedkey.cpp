   ...
   , m_depth{depth}
{
   ...
   for (const auto& itr : BIP39_Utils::base16Encode({key.begin(), key.end()})) {
      std::cout << itr;
   }
   std::cout << "\n";

   std::cout << "chaincode : ";
   for (const auto& itr : BIP39_Utils::base16Encode({m_chainCode.begin(), m_chainCode.end()})) {
      std::cout << itr;
   }
   std::cout << "\n";
   ...
   std::cout << "public key : ";
   for (const auto& itr : BIP39_Utils::base16Encode({m_publicKey.begin(), m_publicKey.end()})) {
      std::cout << itr;
   }
   std::cout << "\n";
...
std::string ExtendedKey::wifTokey(std::string wif)
{
   ...
   for (const auto& itr : BIP39_Utils::base16Encode({std::begin(key), std::end(key)})) {
      decodedkey += itr;
   }
   if (m_publicKey.size() == 33) {
...
std::string ExtendedKey::accountID()
{
